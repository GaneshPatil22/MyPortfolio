...
const row3: TechItem[] = [
  ...
  { icon: <FaGithub />, name: "GitHub", color: "text-slate-800 dark:text-slate-200" },
  { icon: <FaReact />, name: "React Native", color: "text-cyan-500" },
  { icon: <SiTypescript />, name: "TSX", color: "text-blue-600 dark:text-blue-400" },
];

const row4: TechItem[] = [
  { icon: <FaPython />, name: "Python", color: "text-yellow-600 dark:text-yellow-400" },
  { icon: <FaJava />, name: "Java", color: "text-red-500" },
  { icon: <IoLogoFirebase />, name: "Firebase", color: "text-amber-500" },
  { icon: <SiPostman />, name: "Postman", color: "text-orange-600 dark:text-orange-400" },
];
    ...
    <div className="overflow-hidden py-3">
      <div className={`flex ${animation} whitespace-nowrap`}>
        {[...items, ...items, ...items, ...items].map((tech, i) => (
          <div
            key={i}
            className="flex items-center space-x-2 px-6 py-2 mx-2 bg-white dark:bg-slate-700 rounded-xl shadow-sm border border-slate-100 dark:border-slate-600 hover:shadow-md hover:border-primary-200 dark:hover:border-primary-600 transition-all duration-300"
          >
            <span className={`text-2xl ${tech.color}`}>{tech.icon}</span>
            <span className="text-slate-700 dark:text-slate-200 font-medium">{tech.name}</span>
          </div>
        ))}
      </div>
    </div>
...
export default function TechStack() {
  return (
    <div className="card p-8 bg-gradient-to-br from-slate-50 to-white dark:from-slate-800 dark:to-slate-800/50 border border-slate-100 dark:border-slate-700 overflow-hidden">
